...
3. `platform/lib/skill_runner/wrapper.py` from prompt 09a — confirm the pack shape matches what the wrapper expects.
...
1. Validate the pack's structure using the pack_loader (from prompt 09a's `platform/lib/skill_runner/pack_loader.py`). All manifest fields present; schemas parse; handler.py importable.
...
poetry run python -c "
from platform.lib.skill_runner.pack_loader import PackLoader
