...
- **`platform/events/log.py`**: the `EventLog` class. Append-only, SQLCipher-encrypted, partitioned by `owner_scope`. Writes are atomic; reads are read-committed; replay is deterministic.
- **`platform/events/bus.py`**: the `EventBus` class. In-process pub/sub on top of the log. Subscribers register for event types; the bus fans out new events to them and tracks a checkpoint per subscriber so they can resume after restart.
...
- Encryption key: the `EventLog.__init__` receives raw bytes. A separate helper `platform/lib/crypto.py::derive_event_log_key(op_ref: str) -> bytes` reads from 1Password. Stub that helper if 1Password CLI isn't wired up yet.
- Migrations: table `_schema_version` tracks which migration has run. On init, run any pending. Each migration is a SQL file in `platform/events/migrations/0001_initial.sql`, `0002_add_correlation_index.sql`, etc.
...
- Each subscriber has a checkpoint persisted in a separate sqlite db
  (platform/events/bus_checkpoints.db, unencrypted — it's just cursor state).
...
from platform.events.log import EventLog
...
git add platform/events/ tests/unit/test_event_log.py tests/unit/test_event_bus.py scripts/demo_event_log.py
