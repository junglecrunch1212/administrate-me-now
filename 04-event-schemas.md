...
4. `platform/events/log.py` (from prompt 03) — you need to know exactly what shape events take at the storage layer.
...
class SchemaRegistry:
    ...
    def autoload(self) -> None:
        """
        Walks platform.events.schemas package, imports every submodule.
        Each submodule is expected to call registry.register(...) at import time.
        """
        import platform.events.schemas as schemas_pkg
        for _, name, _ in pkgutil.walk_packages(schemas_pkg.__path__, prefix="platform.events.schemas."):
...
### `platform/events/schemas/*.py`

Define exactly these **15 canonical schemas** (one per file, grouped sensibly):

**Ingest / adapter-emitted (`platform/events/schemas/ingest.py`):**
...
**CRM / party-related (`platform/events/schemas/crm.py`):**
- `party.created` v1
...
**Pipeline-emitted domain (`platform/events/schemas/domain.py`):**
...
**Governance (`platform/events/schemas/governance.py`):**
- `observation.suppressed` v1 — outbound intercepted by observation mode

**Also:** `platform/events/schemas/__init__.py` is empty or just docs. Each module, on import, calls `registry.register(...)` for its schemas.
...
Example — `platform/events/schemas/domain.py` (excerpt for one schema):

```python
from pydantic import BaseModel, Field
from datetime import datetime
from typing import Literal
from platform.events.registry import registry
...
Modify `platform/events/log.py::EventLog.append()` to call `registry.validate(type, version, payload)` before inserting. If validation fails, raise `EventValidationError` wrapping the Pydantic error. The log does NOT write invalid events.
...
```bash
# Lint + types
poetry run ruff check platform/events/
poetry run mypy platform/events/
...
git add platform/events/envelope.py platform/events/registry.py platform/events/schemas/
