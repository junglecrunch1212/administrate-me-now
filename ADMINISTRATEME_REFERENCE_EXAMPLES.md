...
3. **Anything hardcoded to Stice-specific identifiers below is acceptable in the pack content.** Pack content is per-instance by design. What is *not* acceptable is hardcoded tenant-specific data in `platform/`, `bootstrap/`, `profiles/` (the *built-in* profiles), `personas/` (the *built-in* personas), `integrations/` (the *built-in* adapters), or `tests/`. The identity scan (`tests/unit/test_no_hardcoded_identity.py`) enforces this.
...
**Pack root.** This is a built-in projection, not a third-party pack. Lives in `platform/projections/parties/`.

### Schema

```sql
-- ~/.adminme/platform/projections/parties/schema.sql
...
```python
# platform/projections/parties/handlers.py
"""
...
```python
# platform/projections/parties/queries.py
"""
...
`platform/events/schemas/commitment.proposed.v3.json`
