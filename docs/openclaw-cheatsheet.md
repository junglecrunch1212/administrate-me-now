...
Two paths. (a) Ship skills as a directory under a skill-loader location and let OpenClaw discover them at load time. Per `docs/reference/openclaw/tools/skills.md` Locations-and-precedence, order is: `<workspace>/skills` > `<workspace>/.agents/skills` > `~/.agents/skills` > `~/.openclaw/skills` > bundled > `skills.load.extraDirs`. (b) Publish to ClawHub and install with `openclaw skills install <slug>` (per `tools/skills.md` ClawHub section + `tools/clawhub.md`) — lands in the active workspace `skills/`. Plugins may also ship skills via `openclaw.plugin.json` `skills` dirs. Config toggles live under `skills.entries.<name>.{enabled,env,apiKey,config}`. A gateway restart is not required unless the skills watcher is off. **Note (Amendment-2):** AdministrateMe adapter packs (`kind: adapter`) install through the same pack-registry infrastructure described above; the five-category epistemic taxonomy ([D19] — Cat-A through Cat-E) refines what an adapter manifest declares (capabilities, write_capabilities, observation_mode_required, sensitivity_default, owner_scope_default) but does not change the install lifecycle. See BUILD.md §ADAPTER FRAMEWORK for the manifest schema; see `openclaw plugins install` (Q4 below) for plugin-shaped channel adapters specifically.
