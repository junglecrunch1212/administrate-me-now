...
## Section 5b: Lists — first-class projection, distinct from tasks

The `lists` projection is the 13th first-class projection per [D18] — household-mirrored lists from Cat-B external-state-mirror adapters (Apple Reminders, Google Tasks, Apple Notes-checklists) plus CoS-native lists. Lists are NOT tasks; the two projections collaborate but do not collapse. [arch §4 row 3.13, BUILD.md §3.13, BUILD.md §LISTS, D18]

1. A **list item** is not a task; tasks remain the obligation-and-work projection (§4) with their full ADHD-neuroprosthetic field set (energy, effort, micro_script, waiting_on, goal_ref, life_event), and list items remain mirror-rows of external-surface state with structurally simpler shape (body, status, position, optional notes). Modeling a grocery line as a task is a category error. [arch §4 row 3.5, arch §4 row 3.13, D18]
2. **Promotion is one-way and non-mutating.** When a list item is promoted to a task, the source list item is unchanged in the `lists` projection — its open/completed state continues to track upstream-mirror semantics; the resulting task is a new row in `tasks` carrying `source_list_item_id` provenance via the `list_item.promoted_to_task@v1` event. The `tasks.created_by` field marks the promotion source; `list_items.promoted_task_id` cross-links back. [BUILD.md §LISTS, D18]
3. **External SSOT inversion.** For Cat-B-sourced lists, the external surface (Reminders.app, Google Tasks UI, Notes-checklist) is the source of truth; the `lists` projection is a mirror. Conflict resolution is last-writer-wins on round-trip per the per-source-kind capability matrix; the projection records `last_event_id` so debug tooling can replay the divergence. CoS-native lists (`source_kind = 'cos_native'`) ARE source-of-truth on the AdministrateMe side; they have `external_id_kind = NULL`. [BUILD.md §LISTS, D18, PM-31]
4. **Deduplication invariant.** The `(external_id_kind, external_list_id)` tuple is unique on `lists` and `(list_id, external_item_id)` is unique on `list_items`; SQLite NULL-distinct semantics correctly let multiple `cos_native` rows coexist with NULL on the dedup columns. The `parties` projection's identifier dedup at [§3] is the precedent pattern. [BUILD.md §LISTS, D18, PM-31]
5. **Write-back asymmetry per source-kind.** Each Cat-B adapter declares `write_capabilities` as an explicit per-operation list (`create`, `update`, `delete`, `toggle_completion`, `reorder`, `add_item`, `remove_item`), not a boolean `bidirectional`. Apple Reminders supports full CRUD; Google Tasks supports full CRUD; Apple Notes-checklists supports `toggle_completion` + `add_item` only (per UT-19 / D18) — no remove, no in-place text edit, no reorder. CoS-side requests for an unsupported capability are rejected at the request layer with a typed error; surfaces gate UI affordances by reading the adapter's manifest. [BUILD.md §APPLE NOTES-CHECKLISTS CROSS-CAT EXTENSION, D18, UT-19, PM-33]
6. **Owner-scope × sharing-model interaction.** Every `lists` row carries a `sharing_model` discriminator (`private` | `shared_household` | `icloud_shared_list` | `google_shared_list`). The `owner_scope` field follows the standard projection convention (`private:<member>` | `shared:household` | `shared:org:<id>`); `sharing_model` is the orthogonal axis describing **how** the sharing is implemented in upstream — an iCloud-shared-list is `shared:household` in scope but `icloud_shared_list` in mechanism, and that mechanism dictates which bridge sees which list updates. [BUILD.md §LISTS, D18, §6.19]
7. **Subscription discipline.** The `lists` projection subscribes to `list.*` and `list_item.*`. The `tasks` projection subscribes to `task.*` and `list_item.promoted_to_task` (the latter to materialize promoted tasks). The `tasks` projection no longer subscribes to `reminder.*` — `reminder.*` is retired per [D18] (UT-22 closure). [arch §3 canonical topic prefixes, arch §4 row 3.5, arch §4 row 3.13, D18, UT-22]
...
19. Each member bridge runs only its assigned member's adapter set; a bridge Mac Mini that runs more than one member's adapter set is a misconfiguration. The central CoS Mac Mini never signs into a family member's iCloud account; cross-member knowledge access happens only through projection queries on the central CoS Mac Mini, never through bridge-to-bridge access. Kid bridges run Apple Notes + Voice Memos only — Obsidian is excluded for child-owned bridges. This is the physical-layer reinforcement of [§6.12] identity-first privacy. [BUILD.md §MEMBER BRIDGES, D17]

**Observation mode at the action verb (Cat-E):**

20. Cat-E (Outbound-Action) adapters integrate observation mode at the **action verb** level via the same `outbound()` seam Cat-A messaging adapters use at the message verb level; both routes pass through `adminme/lib/observation.py` as the single enforcement point. The framework default for any capability declared `kind: cat_e_outbound_action` is `observation_mode_required: true` — the install-time validator (per BUILD.md §ADAPTER FRAMEWORK §3.4) refuses to register a Cat-E capability with `observation_mode_required: false` unless the manifest explicitly justifies the override (e.g. an internal-only Cat-E adapter that has no external side effect). When `observation_mode = active`, a Cat-E request emits `observation.suppressed@v1` with the full would-have-sent payload (service domain, action verb, request body, target entity, requesting actor, source `event_id`) and does NOT emit `action.executed@v1`. Reading external state via Cat-C capabilities is internal logic, not external side effect — Cat-C capabilities declare `observation_mode_required: false` and are unaffected. [arch §6, BUILD.md §ADAPTER TAXONOMY, BUILD.md §ADAPTER FRAMEWORK, D24, PM-32]
...
9. L1 adapters run in **two places**: central adapters (messaging, calendaring, financial, etc.) run as standalone Python processes on the CoS Mac Mini at `:333x` process scope, and bridge adapters (knowledge-source: Apple Notes, Voice Memos, Obsidian, connector packs) run on member bridges per BUILD.md §MEMBER BRIDGES. Bridges emit owner-scoped events into the central event log via the `:3337 bridge` HTTP ingest endpoint over the tailnet; Tailscale identity binds the inbound owner_scope at the endpoint. The bridge daemon does not hold the AdministrateMe SQLCipher master key and does not write to the central event log directly. [BUILD.md §MEMBER BRIDGES, D17, arch §1, arch §9]

10. Adapter categorization is by **epistemic role** (five categories: Cat-A Communication, Cat-B External-State-Mirror, Cat-C Inbound-Only Data, Cat-D Personal-Knowledge, Cat-E Outbound-Action), not by runtime substrate. Runtime is an orthogonal secondary axis: a single category may have central, bridge, or dual-deployment runtime. **Multi-capability adapters** declare each capability as its own seam with its own `kind`, `event_families`, `write_capabilities`, `sensitivity_default`, and `observation_mode_required` declarations; capabilities are a list, not a singleton. Apple Notes-checklists declares two capabilities (Cat-D prose + Cat-B checklist round-trip); Home Assistant declares two capabilities (Cat-C state-read + Cat-E service-call). Install-time validator enforces capability-runtime coherence + base-class conformance (per BUILD.md §ADAPTER FRAMEWORK §3.4). [BUILD.md §ADAPTER TAXONOMY, BUILD.md §ADAPTER FRAMEWORK, D19, D24, PM-35]
