...
**Last updated:** 2026-04-26 (sequence-update PR #37 `sequence-update-10b-split` merged — splits prompt 10b into 10b-i (identity_resolution + noise_filtering) and 10b-ii (commitment_extraction + thank_you_detection) per the split memo at `docs/01-split-memo-10b.md`. Three changes in one commit: PROMPT_SEQUENCE.md sequence-table row for 10b replaced with rows for 10b-i and 10b-ii; dependency graph updated to `10a → 10b-i → 10b-ii → 10c → 10d`; hard-sequential-dependency line updated to note "10b-ii consumes identity_resolution output" + "downstream prompts that depended on 10b now depend on 10b-ii"; `prompts/10b-reactive-pipelines.md` deleted (the unrefactored 26-line draft is superseded by the upcoming split sub-prompts). Partner session ran Type 2 QC: contract check Match (three changes specified, three changes landed; row formatting matches existing table style); invariant audit Clean (zero code touched, no `verify_invariants.sh` invocation needed); next-prompt calibration **Needs refresh** — split memo's "Scope" lines say `packs/pipelines/<n>/` while 10a's `PipelineRunner.discover(builtin_root, installed_root)` walks `adminme/pipelines/` as builtin and `instance_config.packs_dir / "pipelines"` as installed; the 10b-i refactor session must depth-read 10a's discover signature to confirm whether `packs/pipelines/` is the `installed_root` second arg (probable, mirroring 09b's `packs/skills/`) or a path mismatch needing reconciliation. Logged for the 10b-i refactor session, not blocking. **Branch reassignment observed:** harness assigned `claude/sequence-update-10b-split-OFrFL` instead of the requested `sequence-update-10b-split` — Claude Code did not fight it, correct preamble discipline. PM-22 added (sidecar transcripts that omit BUILD_LOG by design are a recognized class — sequence updates and split-memo prep PRs are infrastructure, not build prompts; recorded in this section of `build_log.md` but no full ledger entry). Next refactor target advances to **10b-i** (identity_resolution + noise_filtering pipelines + classify_message_nature skill pack + identity.merge_suggested + messaging.classified event schemas at v1). Prior session's 10a + sidecar PR #35 findings retained immediately below.

**Last updated:** 2026-04-26 (sidecar PR #35 `sidecar-raw-data-is-manual-derived` merged — closes 07.5 finding C-1. Two-file change: `adminme/projections/xlsx_workbooks/sheets/raw_data.py` ALWAYS_DERIVED now includes `"is_manual"` matching the descriptor's `always_derived`; new `test_raw_data_always_derived_matches_descriptor` in `tests/unit/test_xlsx_finance_workbook.py` is the both-direction drift canary. Partner session ran Type 2 QC: contract check Match (two-file scope respected, no creep); invariant audit Clean (no new event types, no SDK imports, no path literals, no tenant identity, `verify_invariants.sh` exit 0); next-prompt calibration Clean (10b unaffected — sidecar touches Raw Data builder + finance-workbook test, neither in 10b's Read first). Suite tally 411 → 412 passed, 1 skipped (added 1 sync test to the finance-workbook file). UT-1 confirmed CLOSED. Prior session's 10a merge findings retained immediately below.

**Last updated:** 2026-04-26 (10a merged as PR #33 — pipeline runner per BUILD.md §L4 landed; reactive-only subscription machinery + pack loader + Pipeline Protocol + 17 new tests + verify_invariants.sh's pipeline→projection canary armed-and-clean. Partner session of 2026-04-26 is Type 1 — QC of 10a merge + housekeeping. **Findings on 10a merge: Clean** on all three jobs. Contract check: prompt floor 14 tests; PR shipped **17** (overshoot from 3 extra `pack_loader` cases — wrong-kind manifest, missing-entrypoint-file, class-without-handle protocol-check). Overshoot logged as quality signal. Silent architecture decision: prompt's Commit 1 named test-#6 as "missing class OR missing handle"; Claude Code factored into two separate tests for cleaner separation. Accepted. Invariant audit: clean — `verify_invariants.sh` exits 0; pipeline→projection canary remains vacuous (echo_logger and echo_emitter fixtures don't write projections; `EchoEmitterPipeline` emits `messaging.sent` via `event_log.append`, the right seam); zero LLM/embedding SDK imports anywhere under `adminme/pipelines/` or `tests/fixtures/pipelines/`; zero hardcoded instance paths; zero tenant identity; zero new system events registered (echo_emitter reuses `messaging.sent` v1 per the prompt's UT-2 carve-out, no new schema needed). Single-seam check: `[ADR-0002]` `PipelineContext.run_skill_fn` confirmed as the only seam pipelines can use to call skills; runner code itself does not POST to OpenClaw. Carry-forward set into 10b/10c/16/19 in build_log entry intact. Next-prompt calibration on **10b**: pre-split candidate per `D-prompt-tier-and-pattern-index.md`. Next Partner session is the 10b refactor — startup report MUST forecast the split (10b-i identity_resolution + noise_filtering / 10b-ii commitment_extraction + thank_you_detection) before drafting.)
...
**Prompts drafted, ready for Claude Code execution:** none. The 10b split memo is on disk at `docs/01-split-memo-10b.md` (Tier C memo; no sub-prompts drafted yet). Next refactor target: **10b-i** (identity_resolution + noise_filtering pipelines + classify_message_nature skill pack + identity.merge_suggested + messaging.classified event schemas at v1).

**Sidecar PRs queued (non-blocking):** none. Most recent sidecar `sidecar-raw-data-is-manual-derived` merged as PR #35 on 2026-04-26 (closed 07.5 finding C-1). Most recent sequence update `sequence-update-10b-split` merged as PR #37 on 2026-04-26.

**Next task queue (in order):**

1. **Partner session: refactor 10b-i (identity_resolution + noise_filtering).** Per `docs/01-split-memo-10b.md` §10b-i scope: pipeline pack `identity_resolution` (heuristic-only — no skill calls; subscribes to `messaging.received` / `messaging.sent` / `telephony.sms_received`; emits `identity.merge_suggested` above 0.85 similarity, otherwise `party.created` + `identifier.added`); pipeline pack `noise_filtering` (calls `classify_message_nature@v2` skill; emits `messaging.classified` with classification + confidence + skill version); skill pack `packs/skills/classify_message_nature/` (full 09b shape; classifies as `noise | transactional | personal | professional | promotional` per BUILD.md §1136); two new event schemas at v1 (`identity.merge_suggested` next to `party.merged` in `crm.py`; `messaging.classified` — module choice deferred to depth-read). **Calibration finding from PR #37 QC carries forward:** the 10b-i refactor session must depth-read 10a's `PipelineRunner.discover(builtin_root, installed_root)` signature in `adminme/pipelines/runner.py` and confirm whether `packs/pipelines/<n>/` (the path the split memo uses) is the `installed_root` second argument or a path mismatch needing reconciliation — 09b's `packs/skills/` precedent says probable, but verify against shipped 10a code before drafting the refactored prompt's Read first / Deliverables. Refactor must (a) prepend the slim post-PM-7 preamble verbatim, (b) update `Read first` against shipped 10a code (`adminme/pipelines/{base,pack_loader,runner}.py`), (c) confirm each pipeline's `pipeline.yaml` lives at the path 10a's discover() expects, (d) confirm pipelines emit derived events with `causation_id=ctx.triggering_event_id` per the echo_emitter canary, (e) for noise_filtering, confirm the skill-call path: `await ctx.run_skill_fn("classify_message_nature", inputs, SkillContext(session=ctx.session, correlation_id=ctx.correlation_id))`, (f) `identity.merge_suggested` and `messaging.classified` register at v1 per [D7]; verify no name collision with `noise.filtered` (the original draft used `noise.filtered` but BUILD.md §1136 says `messaging.classified` — split memo already corrected), (g) tenant-identity discipline — pipeline code must NOT reference James/Laura/Charlie/Stice/Morningside even illustratively; fixtures live at `tests/fixtures/` with `# fixture:tenant_data:ok` markers. Estimated session size per split memo: ~450-line refactored prompt; ~30–35 tests; one new skill pack; two pipeline packs; two event schemas. Fits one Claude Code session.
2. **Claude Code session: execute 10b-i.** James drives. Ships pipeline directories + skill pack + test pyramids + new event schemas at v1.
3. **Partner session: QC of 10b-i merge + refactor 10b-ii (commitment_extraction + thank_you_detection).** Per split memo: three new skill packs (`classify_commitment_candidate`, `extract_commitment_fields`, `extract_thank_you_fields`); two pipeline packs; one new event schema (`commitment.suppressed` at v1); ~500-line refactored prompt; ~35–45 tests. Watch flag from split memo: if depth-read at refactor time reveals 10b-ii is still too big, it itself becomes a candidate for 10b-ii-α / 10b-ii-β.
4. **Partner session: QC of 10b-ii merge + refactor 10c (proactive pipelines).** 10c is also a pre-split candidate per `D-prompt-tier-and-pattern-index.md` (six proactive pipelines); pre-split forecast goes in startup report. **UT-2 must be resolved before 10c can be refactored** — the AGENTS.md-prose + `openclaw cron add` registration path is currently underspecified. Resolution comes from depth-reading `docs/reference/openclaw/automation/standing-orders.md` + `docs/reference/openclaw/concepts/agent-workspace.md` and codifying whether bootstrap §8 concatenates per-pipeline markdown into AGENTS.md or ships AGENTS.md pre-written.
...
…and find-and-replace `<PR-09b>`, `<commit4-09b>`, `<merge-date-09b>` in `docs/build_log.md` and `docs/partner_handoff.md` with the actual values. Same applies to the still-pending `<PR-08a>` / `<PR-08b>` / `<sha1-08a>` etc. placeholders inherited from prior sessions.

**Prompts drafted but not yet refactored:** 10b-i, 10b-ii (both pending after the 10b split), 10c, 10d, 11, 12, 13a, 13b, 14a, 14b, 14c, 14d, 14e, 15, 15.5, 16, 17, 18, 19. (10a moved to "merged to main" 2026-04-26; 10b retired per PR #37 in favor of 10b-i / 10b-ii.) The slim preamble means each refactor is shorter than 07a/07b were. 15, 16 remain pre-split candidates; per `D-prompt-tier-and-pattern-index.md`, 10c, 11, 14b, 17 are also pre-split candidates.

**Note on on-disk vs. shipped 10a:** the file `prompts/10a-pipeline-runner.md` on main is still the unrefactored 90-line v1 draft. Claude Code executed against the **refactored ~340-line prompt that Partner held in chat and James pasted in** — never committed to repo. This is the standard pattern (per workflow norms, refactored prompts are paste-only) but worth flagging because the build_log entry references `prompts/10a-pipeline-runner.md (~340 lines)` which does NOT match what's on disk. **Action item (next session, if cheap):** decide whether to start committing refactored prompts to `prompts/` so the file-on-disk matches the executed-prompt evidence in build_log. Currently logged as PM-21, not a hard finding — historical PRs all followed the same pattern, which has been working.

**Prompts not yet drafted:** 10b-i and 10b-ii (sub-prompts from the 10b split; the split memo at `docs/01-split-memo-10b.md` is the scope source — per-sub-prompt drafts pending Partner refactor sessions). Everything else from 11 onward exists in unrefactored form. The original `prompts/10b-reactive-pipelines.md` was retired as part of PR #37.
...
### PM-21: Refactored prompts are paste-only by convention; the on-disk file in `prompts/<NN>-<slug>.md` may lag behind what Claude Code executed against — SOFT
...
│   ├── pipelines/                              # MERGED 10a (base.py, pack_loader.py, runner.py); 10b-i/10b-ii will populate (or packs/pipelines/ — UT-11)
