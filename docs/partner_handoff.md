...
**Last updated:** 2026-04-28 (09b merged as PR #<PR-09b> — first canonical skill pack `classify_thank_you_candidate` v1.3.0 landed; pack scaffold + handler safety-net + `bootstrap/pack_install_order.yaml` shipped. Partner Session of 2026-04-28 was Type 1 — QC of 09b merge. **Findings: Clean** on all three jobs. Contract check: prompt floor 5 tests (4 integration + 1 unit canary); PR shipped **8** (4 integration + 4 unit — overshoot from non-dict defensive + well-formed pass-through + missing-urgency coercion handler-direct cases in `packs/skills/.../tests/test_skill.py`). Overshoot logged as quality signal. Invariant audit: clean — `verify_invariants.sh` exits 0; zero LLM/embedding SDK imports added under `adminme/` or `packs/`; zero hardcoded instance paths; tenant-identity strings (stice-james / Klein / Mike) confined to `packs/skills/classify_thank_you_candidate/tests/fixtures/` and `tests/integration/test_classify_thank_you_pack.py`; zero domain-event additions, zero `verify_invariants.sh` edits — pack is purely a wrapper-consumer per the prompt's Out of scope. Next-prompt calibration on **10a**: `prompts/10a-pipeline-runner.md` is 90 lines, unrefactored v1 long form (`**Phase:** BUILD.md L4`, no slim preamble, "Depends on: Prompt 09b" symbolically correct). **Finding: Needs refactor** — whole-prompt redo for slim-preamble migration + Session/guardedWrite/outbound threading from 08a/08b + `run_skill()` integration from 09a + `InstanceConfig.packs_dir` absolute-path discipline from UT-9. Next session is the 10a refactor.)

This section is the live baton between sessions. Update it at the end of every Partner session.

**Prompts merged to main:** 00, 00.5, 01 (01a/01b/01c), 02, 03, 03.5, 04, 05, 06, 07a, 07b, **PM-7 infrastructure PR (slim preamble + scripts/verify_invariants.sh)**, **07c-α (PR #20, merged 2026-04-24)**, **07c-β (PR #21, merged 2026-04-25 — reverse daemon class + 4 emit pathways + integration round-trip; closes the xlsx round-trip and resolves UT-6)**, **08a (PR #&lt;PR-08a&gt;, merged 2026-04-25 — Session model + scope enforcement; 48 TODO(prompt-08) markers cleared across 10 sqlite projection queries.py files; 69 new tests; resolves UT-8 inline via `vector_search.nearest` three-layer carve-out)**, **08b (PR #&lt;PR-08b&gt;, merged 2026-04-25 — guardedWrite three-layer + observation `outbound()` + 6 governance event types at v1; 47 new tests + 4 security-E2E + 1 UT-7 closure case; resolves UT-7 — `_ACTOR` literal removed from reverse daemon, sidecar hedge NOT activated)**, **09a (PR #29, merged 2026-04-26 — skill runner wrapper around OpenClaw `/tools/invoke` + `llm-task`; 30 new tests; ADR-0002 schema relaxation folded in per PM-19; `verify_invariants.sh` extended with `skill.call.*` single-seam check)**, **09b (PR #&lt;PR-09b&gt;, merged &lt;merge-date-09b&gt; — first canonical skill pack `classify_thank_you_candidate` v1.3.0; 8 new tests (4 unit + 4 integration); `bootstrap/pack_install_order.yaml` queued for prompts 15/16; zero domain events, zero `verify_invariants.sh` edits — pure wrapper-consumer)**.
...
**Prompts drafted, ready for Claude Code execution:** none. Next refactor target: **10a** (pipeline runner per BUILD.md L4 — first prompt that consumes the 09a/09b skill-runner stack from inside reactive pipelines).
...
1. **Sidecar Claude Code session: ship `sidecar-raw-data-is-manual-derived`** — non-blocking, still queued; carried forward from the 07.5 audit (C-1 finding). Two-file change in `adminme/projections/xlsx_workbooks/sheets/raw_data.py:45` + a unit test asserting equivalence with `descriptor_for(FINANCE_WORKBOOK_NAME, "Raw Data").always_derived`. ≤15 minutes. Cosmetic protection drift; does not block prompt 10a.
2. **Partner session: refactor 10a (Pipeline runner — Type 3, refactor-only).** Draft exists at `prompts/10a-pipeline-runner.md` in v1 long form (90 lines, no slim preamble; `**Phase:** BUILD.md L4` prose header). Pre-split disposition per `D-prompt-tier-and-pattern-index.md`: single prompt expected (Tier B, pattern Introduction — NOT a pre-split candidate; 10b and 10c are pre-split candidates downstream). Refactor must (a) prepend the slim post-PM-7 preamble verbatim, (b) update `Read first` to depth-read `adminme/lib/skill_runner/wrapper.py` (`run_skill` signature + `SkillContext` shape + `set_default_event_log`) and `adminme/lib/skill_runner/pack_loader.py` (pack-shape contract for the *pipeline-pack* analogue — note: pipeline packs are a *new* shape, not the skill-pack shape; verify what BUILD.md L4 actually specifies vs. what 09a's loader handles), (c) thread `Session` from `adminme.lib.session` (build via `build_internal_session("pipeline_runner", "device", tenant_id)` per 08a's carry-forward), (d) thread `outbound()` from `adminme.lib.observation` for any pipeline that emits external side effects, (e) thread `guarded_write.check(session, "<action>", ...)` from `adminme.lib.governance` before pipeline-emitted domain events that affect external state, (f) confirm pipeline-runner consumption of `bootstrap/pack_install_order.yaml` is **NOT** in 10a's scope per 09b's carry-forward — that file is consumed in prompt 15/16, (g) per UT-9 SOFT, pipeline runner should construct absolute pack paths via `InstanceConfig.packs_dir` rather than relying on the slug-fallback in `_resolve_pack_root`, (h) confirm `docs/reference/openclaw/` has the standing-order registration API files that 10a's `Read first` §3-§4 depend on; if not, prompt 00.5 sidecar before 10a can ship. Job 3 size budget: target ~220–260 lines slim form (well under 350-line ceiling). Single-prompt expected; if depth-read surfaces a split, write a Tier C split memo first per E-session-protocol §Splits.
3. **Claude Code session: execute 10a.** James drives. Ships `adminme/lib/pipeline_runner/` (or wherever the BUILD.md L4 spec places it) + a trivial pipeline pack to prove end-to-end + reactive-bus subscription + restart survival.
4. **Partner session: QC of 10a merge + refactor 10b (reactive pipelines).** 10b is a pre-split candidate per `D-prompt-tier-and-pattern-index.md`; pre-split forecast goes in the startup report; if Job 2 depth-read confirms split, the session's primary artifact is the split memo, not a drafted prompt.
...
### UT-9: Pack-root resolution accepts three forms; production should converge on one — SOFT

Introduced in 09a. `adminme/lib/skill_runner/[wrapper.py](http://wrapper.py)::_resolve_pack_root` accepts three forms of `skill_id`: absolute path, repo-relative slug (test convenience), and `namespace:name` form. Production callers (pipeline runner from 10a onward) should pass absolute paths derived from `InstanceConfig.packs_dir` so the resolver only sees one shape in production. Track during 10a refactor: confirm pipeline runner constructs absolute paths and does not depend on the slug fallback. If 10a confirms, retire the slug fallback in a follow-up sidecar (test-only fixture packs would need to switch to absolute fixture paths, which they already use — `FIXTURE_DIR / "scope_required_pack"`).
