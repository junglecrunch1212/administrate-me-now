...
**Prompts drafted, ready for Claude Code execution:** none. Next refactor target: **10b-ii** (commitment_extraction + thank_you_detection pipelines + three new skill packs + commitment.suppressed event schema at v1). Per `docs/01-split-memo-10b.md` §10b-ii. **UT-12 must be evaluated at 10b-ii orientation** (parties-DB seam decision — see Open tensions).
...
1. **Partner session: refactor 10b-ii (commitment_extraction + thank_you_detection).** Per `docs/01-split-memo-10b.md` §10b-ii scope: pipeline pack `commitment_extraction` (subscribes to `messaging.received` + `messaging.sent` + (TODO: telephony.voicemail_transcribed, calendar.event.concluded, capture.note_created when registered); resolves sender via `find_party_by_identifier`; calls `classify_commitment_candidate` then on positive `extract_commitment_fields`; emits `commitment.proposed` (existing v1) or `commitment.suppressed` (new v1); honors REFERENCE_EXAMPLES.md §2 dedupe_window_hours per-member overrides); pipeline pack `thank_you_detection` (subscribes to same; calls existing `classify_thank_you_candidate@1.3.0` from 09b; on positive calls `extract_thank_you_fields`; emits `commitment.proposed` with kind=other + reasons referencing the thank-you decision OR extends kind enum if depth-read reveals it must); three new skill packs in 09b shape (`classify_commitment_candidate`, `extract_commitment_fields`, `extract_thank_you_fields`); one new event schema (`commitment.suppressed`) at v1 in `domain.py` next to existing `commitment.proposed`. **UT-12 (parties-DB seam) MUST be resolved at orientation BEFORE Job 2 drafting begins** — three options on the table; pick (a), (b), or (c). **F-2 carry-forward from 10b-i QC:** verify the three new skills' `sensitivity_required` and `context_scopes_required` and either match 10b-i's normal-empty shape or widen `except` lists in both pipelines to catch `SkillSensitivityRefused` and `SkillScopeInsufficient`. Estimated session size per split memo: ~500-line refactored prompt; ~35–45 tests; three skill packs; two pipeline packs; one event schema. Watch flag: if depth-read at refactor time reveals 10b-ii is still too big, it itself becomes a candidate for 10b-ii-α / 10b-ii-β.
2. **Claude Code session: execute 10b-ii.** James drives. Ships pipeline directories + three skill packs + test pyramids + new event schema at v1.
3. **Partner session: QC of 10b-ii merge + decide 10c orientation (proactive pipelines).** 10c is also a pre-split candidate per `D-prompt-tier-and-pattern-index.md` (six proactive pipelines); pre-split forecast goes in startup report. **UT-2 must be resolved before 10c can be refactored** — the AGENTS.md-prose + `openclaw cron add` registration path is currently underspecified. Resolution comes from depth-reading `docs/reference/openclaw/automation/standing-orders.md` + `docs/reference/openclaw/concepts/agent-workspace.md` and codifying whether bootstrap §8 concatenates per-pipeline markdown into AGENTS.md or ships AGENTS.md pre-written.
...
Status: **OPEN, must resolve at 10b-ii orientation.**
