...
**Prompts drafted, ready for Claude Code execution:** none. Next refactor target: **10b** (reactive pipelines: identity_resolution + noise_filtering + commitment_extraction + thank_you_detection — pre-split candidate per `D-prompt-tier-and-pattern-index.md`; Job 3 forecast may bisect into 10b-i and 10b-ii).

**Sidecar PRs queued (non-blocking):** none. Most recent sidecar `sidecar-raw-data-is-manual-derived` merged as PR #35 on 2026-04-26 (closed 07.5 finding C-1).

**Next task queue (in order):**

1. **Partner session: refactor 10b (Reactive pipelines).** Draft exists at `prompts/10b-reactive-pipelines.md` in v1 long form. **Pre-split candidate** per `D-prompt-tier-and-pattern-index.md` — four pipelines (`identity_resolution`, `noise_filtering`, `commitment_extraction`, `thank_you_detection`) plus their test pyramids likely exceed Job 3 budgets (>4 net-new modules; per-commit code/test budgets). Job 3 forecast: split into **10b-i** (identity_resolution + noise_filtering — both consume `messaging.received` and don't need a skill pack on disk) and **10b-ii** (commitment_extraction + thank_you_detection — both call `run_skill()`; thank_you_detection consumes the merged `classify_thank_you_candidate` pack from 09b end-to-end). The startup report MUST forecast the split before drafting; if depth-read confirms, the session's primary artifact is a Tier C split memo, not a fully-drafted single prompt. Refactor must (a) prepend the slim post-PM-7 preamble verbatim, (b) update `Read first` against shipped 10a code (`adminme/pipelines/{base,pack_loader,runner}.py`), (c) confirm each pipeline's `pipeline.yaml` lives at `adminme/pipelines/<namespace>/<n>/` (built-in path that `PipelineRunner.discover()` walks), (d) confirm pipelines emit derived events with `causation_id=ctx.triggering_event_id` per the echo_emitter canary, (e) for thank_you_detection, confirm the skill-call path: `await ctx.run_skill_fn("classify_thank_you_candidate", inputs, SkillContext(session=ctx.session, correlation_id=ctx.correlation_id))`, (f) `commitment.proposed` (and any other new domain event) registers at v1 in `adminme/events/schemas/domain.py` per [D7] schema-conventions; verify shape against [REFERENCE_EXAMPLES.md §2 lines 584-700], (g) tenant-identity discipline — pipeline code must NOT reference James/Laura/Charlie/Stice/Morningside even illustratively; fixtures live at `tests/fixtures/` with `# fixture:tenant_data:ok` markers.
2. **Claude Code session: execute 10b (or 10b-i, then 10b-ii).** James drives. Each sub-prompt ships pipeline directories + their test pyramids + new domain events at v1.
3. **Partner session: QC of 10b merge + refactor 10c (proactive pipelines).** 10c is also a pre-split candidate per `D-prompt-tier-and-pattern-index.md` (six proactive pipelines); pre-split forecast goes in startup report. **UT-2 must be resolved before 10c can be refactored** — the AGENTS.md-prose + `openclaw cron add` registration path is currently underspecified. Resolution comes from depth-reading `docs/reference/openclaw/automation/standing-orders.md` + `docs/reference/openclaw/concepts/agent-workspace.md` and codifying whether bootstrap §8 concatenates per-pipeline markdown into AGENTS.md or ships AGENTS.md pre-written.
...
**Prompts drafted but not yet refactored:** 10b, 10c, 10d, 11, 12, 13a, 13b, 14a, 14b, 14c, 14d, 14e, 15, 15.5, 16, 17, 18, 19. (10a moved to "merged to main" 2026-04-26.) The slim preamble means each refactor is shorter than 07a/07b were. 15, 16 remain pre-split candidates; per `D-prompt-tier-and-pattern-index.md`, 10b, 10c, 11, 14b, 17 are also pre-split candidates.
...
09a Commit 4 extends `ALLOWED_EMITS` to permit three new event types from `adminme/lib/skill_runner/[wrapper.py](http://wrapper.py)`. The xlsx single-seam pattern (`ALLOWED_EMIT_FILES`) was carried forward as a comment in 07c-α. 09a ships using the same pattern if the script structure supports it cleanly; if not, falls back to test-side enforcement per PM-17 (the path 08b chose for `external.sent`/`observation.suppressed`). After 09a merges, Partner reviews which path was taken and decides whether to harden the script's per-file allowlist support — a candidate single-purpose PR. Status: **partially resolved 2026-04-26**. 09a took the script-side path: `scripts/verify_[invariants.sh](http://invariants.sh)` now contains a `SKILL_EMITS` block that greps for `[skill.call](http://skill.call).{recorded,failed,suppressed}` outside the wrapper file. Pattern is grep-based (matches `event_type="..."` literals) rather than the more elaborate per-file allowlist 07c-α deferred. Future Partner sessions can lift `external.sent` / `observation.suppressed` from test-side to script-side using this same pattern when convenient. Tracked as a future single-purpose PR; not blocking.
