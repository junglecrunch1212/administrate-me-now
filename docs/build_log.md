...
- **Evidence**:
  ...
  - `tests/unit/test_pipeline_thank_you_detection.py` — 13 handler-direct unit tests covering threshold paths, F-5 outbound early-return, suppression reasons (below_min, is_candidate=False, sender unresolvable, factory missing), skip rules, F-2-widened skill-failure paths (classify timeout, extract failure), per-member overrides (lower-threshold + impossibly-high disable per `REFERENCE_EXAMPLES.md §2 line 666`), and classify-output-shape robustness (urgency/suggested_medium absent on `is_candidate=False` per the upstream JSON Schema if/then).
  - `tests/integration/test_pipeline_10b_ii_beta_integration.py` — 3 round-trip tests against the live runner with a seeded parties DB and monkeypatched skill runner: above-threshold emits `commitment.proposed` with `kind="other"` and causation chain intact; `is_candidate=False` with absent urgency/suggested_medium emits `commitment.suppressed` without crashing; F-5 absence-assertion confirms `messaging.sent` never derives a commitment event (its `event_id` does not appear as any `causation_id` in the log after the follow-up `messaging.received` checkpoint advances).
  - **Total new tests: 21** (4 extract_thank_you_fields + 1 thank_you_detection pack-load + 13 handler-direct unit + 3 integration). Suite tally on the `tests/` testpath: 464 → 480 passed, 2 skipped (the +5 pack-internal tests live under `packs/` and run via explicit path per the per-commit verification commands).
  - **F-5 carry-forward CLOSED**: `thank_you_detection` does NOT subscribe to `messaging.sent` (trigger list is `[messaging.received]` only); the handler additionally early-returns on any non-`messaging.received` event type as defense-in-depth so a future broader subscription cannot generate audit-trail noise. Different from `commitment_extraction` which subscribes to both inbound + outbound and runs outbound through the defensive-default suppression path.
  - **`kind="other"` v1 disposition CONFIRMED**: `BUILD.md §1150` depth-read concluded that "specialization of commitment extraction for gratitude" + "thank_you commitment" reads as descriptive (a commitment that is a thank-you), not as requiring a new value in `CommitmentProposedV1.kind`'s Literal. The kind enum is therefore NOT extended; thank-you commitments emit with `kind="other"` and a `classify_reasons` array conveying the thank-you signals. A future Literal-extension migration may revisit this if downstream consumers need to disambiguate.
  - **Urgency-vocabulary asymmetry handled**: `classify_thank_you_candidate@1.3.0`'s urgency Literal (`within_24h | this_week | within_month | no_rush`) does NOT match `CommitmentProposedV1.urgency`'s Literal (`today | this_week | this_month | no_rush`). The pipeline does not coerce/translate at the pipeline layer — the classify-side urgency is treated as a hint to the extractor only (and not even passed through, since the extractor sees the same message text). `extract_thank_you_fields` produces the canonical urgency value, which round-trips into `CommitmentProposedV1` without coercion drift.
  - `[§7.3]` (no projection direct writes): pipeline emits only via `ctx.event_log.append`; pipeline→projection canary in `verify_invariants.sh` clean.
  - `[§7.4]` / `[§8]` / `[D6]`: zero new SDK imports; the two skill calls go through `ctx.run_skill_fn` per `[ADR-0002]`.
  - `[§7.7]`: skill failure on either call does NOT raise — emits `commitment.suppressed` with `reason="skill_failure_defensive_default"`. Bus checkpoint advances normally.
  - `[D7]`: zero new event-type registrations — reuses `commitment.proposed` v1 (existing) + `commitment.suppressed` v1 (registered in 10b-ii-α).
  - `[§12.4]`: per-member-override config uses placeholder member ids (`member_id_example_*`); verify script clean.
  - `[§15]`/`[D15]`: parties-DB path resolved through `InstanceConfig.projection_db_path("parties")` in the integration test harness; no hardcoded literal in handler.
  - Causation-id wiring: every emit uses `causation_id=ctx.triggering_event_id`.
  - `verify_invariants.sh` exit 0.
  - `poetry run ruff check adminme/ packs/ tests/`: clean. (Pre-existing 2 F401 in `docs/reference/plaid/` carry forward from prior merges and are out-of-scope.)
- **Carry-forward for prompt 10c** (proactive pipelines):
  - The two-skill-chain pattern with binary-classifier-first is now established across two pipelines (`commitment_extraction`: classify-then-extract using same-prompt skill family; `thank_you_detection`: classify-then-extract with cross-prompt classifier from 09b). Future proactive pipelines that need a binary gate before extraction can clone either shape literally.
...
- **Carry-forward for prompt 14b** (inbox surface):
  - `commitment.proposed` events with `kind="other"` carrying a `classify_reasons` array including thank-you signals (e.g. `["hosted hospitality"]`) are visually distinct in the inbox from `kind="reply"`/`task`/`appointment`/etc. The 14b inbox surface UI may want to filter or group thank-you proposals separately. This is presentation, not pipeline.
- **Carry-forward for prompt 16** (bootstrap wizard):
  - Bootstrap §3 collects per-member overrides for `thank_you_detection` alongside `commitment_extraction`. The `config.example.yaml` shape is identical between the two pipelines (`min_confidence`, `review_threshold`, `dedupe_window_hours`, `per_member_overrides`, `skip_party_tags`).
- **Carry-forward for prompt 19** (Phase B smoke test):
...
- **Evidence**:
  ...
  - `packs/pipelines/morning_digest/{pipeline.yaml,handler.py,config.example.yaml,config.schema.json,tests/test_morning_digest_pack_load_pack_internal.py}` — `MorningDigestPipeline` with constructor-injection loaders per PM-27 (mirroring 10c-i's reward_dispatch shape); validation guard per `[BUILD.md §1289]` runs every claimed projection-side id from the skill output against the gather payload, sentinel emits on any miss; single-site `outbound()` call in `deliver()` per `[§6.14]`. Proactive pack: `triggers: {schedule: "0 7 * * *", proactive: true}` with NO `triggers.events` — runner skips per `runner.py:131-138`.
  - `packs/pipelines/paralysis_detection/{pipeline.yaml,handler.py,tests/test_paralysis_detection_pack_load_pack_internal.py}` — `ParalysisDetectionPipeline` deterministic per `[BUILD.md §1297-1302]` and operating rule 20 (`[BUILD.md §124]`); template selection is a SHA-1-seeded round-robin keyed on `(member_id, today_iso)`; NEVER calls `run_skill`; v1 has NO `outbound()` call per `[BUILD.md §1302]`'s "inbox + optional outbound" — the optional-outbound branch is deferred. Proactive pack: `triggers: {schedule: "0 15,17 * * *", proactive: true}`.
  - `bootstrap/openclaw/programs/{morning_digest,paralysis_detection}.md` — STUB → full program per `[D1]` Corollary. Each ≤70 lines; cites `[BUILD.md §1289]` (digest validation guard) and `[BUILD.md §1297-1302]` + operating rule 20 (paralysis determinism). Approval gate, Escalation, Execution steps, What NOT to do filled in; Scope verbatim from 10c-i stubs.
  - `tests/unit/bootstrap/test_openclaw_programs_loadable.py` — `test_reward_dispatch_is_fully_populated` generalized to `test_fully_populated_programs_have_no_todo_markers`; the `FULLY_POPULATED_PROGRAMS` set now includes `morning_digest` + `paralysis_detection`. The `test_stub_programs_carry_todo_markers` test now scopes to the remaining three stubs (`reminder_dispatch` / `crm_surface` / `custody_brief`).
  - **Total new tests: 42** (10 schema-registration + 2 compose_morning_digest skill load + 12 morning_digest unit + 2 morning_digest pack-load + 11 paralysis_detection unit + 2 paralysis_detection pack-load + 3 integration). 10c-i suite tally 508 → 550 passed, 2 skipped on the `tests/` testpath; pack-internal tests (2 morning_digest + 2 paralysis_detection + 3 compose_morning_digest = 7) live under `packs/` and run via explicit path. **Validation-guard floor met**: 3 fabrication-shape tests (calendar, commitment, task) + 1 sentinel-emission test (skill failure path) — see `test_morning_digest_pipeline.py`.
  - `[§6.14]` (single `outbound()` enforcement point): morning_digest's `deliver()` is the single call site; paralysis_detection has zero call sites (v1 surface is inbox-only).
  - `[§7.3]` (no projection direct writes): both pipelines emit only via `ctx.event_log.append`; pipeline→projection canary in `verify_invariants.sh` clean.
  - `[§7.4]` / `[§8]` / `[D6]` (no LLM SDK imports in adminme/): paralysis_detection has no skill calls; morning_digest's compose call goes through `ctx.run_skill_fn` per `[ADR-0002]`.
  - `[§7.7]` (defensive default; bus checkpoint not affected): morning_digest catches the full `_SKILL_FAILURE_EXCEPTIONS` tuple (matching 10b-ii's F-2 widening) and emits the sentinel `digest.composed`. paralysis_detection defensively skips on every pre-condition miss.
  - `[D7]`: both new event types register at v1; class names `DigestComposedV1` / `ParalysisTriggeredV1`.
  - `[§12.4]` (tenant-identity firewall): all member ids in fixtures, configs, and test cases are placeholders ("member-a"/"member-b"/etc.); no real tenant names ship in `packs/`, `bootstrap/`, or `adminme/`.
  - `[§15]`/`[D15]`: paths resolved through `InstanceConfig` in integration tests; no hardcoded literal in handlers.
  - Causation-id wiring: every emit uses `causation_id=ctx.triggering_event_id`.
  - `verify_invariants.sh` exit 0; `poetry run ruff check adminme/ packs/ tests/` clean; `poetry run mypy adminme/` reports 0 issues across 114 source files.
  - **Pack-test collision fix**: pack-internal `test_pack_load.py` files renamed to `test_<pack>_pack_load_pack_internal.py` per pytest's collection rule (test files with the same basename across multiple dirs collide on rootdir-relative module names). Same fix path the next pack-load author will need; flag for prompt 10c-iii.
- **Carry-forward for prompt 10c-iii** (`reminder_dispatch` + `crm_surface` + `custody_brief`):
  - The constructor-injection-loader pattern is now established across three consumers (reward_dispatch + morning_digest + paralysis_detection). Three more packs cloning the same shape literally is the 10c-iii budget.
  - The pack-internal `test_<pack>_pack_load_pack_internal.py` rename pattern carries forward — every new pack-internal pack-load test must use a unique basename.
  - The validation-guard pattern from morning_digest can be cloned directly for any future pipeline that calls a compose skill and needs to verify referenced ids against projection state.
  - The `outbound()` single-site discipline plus `delivered` / `validation_failed` payload fields on the emitted event carry the deliverability semantics out to consumers; clone for `reminder_dispatch` / `crm_surface` / `custody_brief`.
- **Carry-forward for UT-14 loaders** (no specific prompt assigned):
  - Three pipelines now ship with constructor-injection loaders that default to no-op (return `None`/`[]`). When real on-disk loader modules land, the wiring point is the runner's pack instantiation (or a later constructor-injection seam). The integration tests set loaders via attribute assignment on the loaded pack instance — the same hook a future bootstrap-side wiring would use.
- **Carry-forward for prompts 14a/14b** (console framework + views):
  - The console's "Today / morning view" surface consumes `digest.composed` events. The `validation_failed` and `delivered` payload fields let the surface render: (a) sentinel placeholder for `validation_failed=True`; (b) "delivered to channel" indicator for `delivered=True`; (c) "queued in observation mode" for `delivered=False` AND `validation_failed=False`.
  - The inbox view consumes `paralysis.triggered` events. v1 surface is inbox-only per `[BUILD.md §1302]`.
- **Carry-forward for paralysis_detection optional outbound** (deferred per `[BUILD.md §1302]`):
  - v1 has NO `outbound()` call. The optional-outbound branch lands when per-profile config-aware routing is introduced (no specific prompt assigned).
