...
Build `adminme/lib/skill_runner/wrapper.py` + supporting modules. Plus a dummy skill pack `packs/skills/classify_test/` used for testing.
...
### `adminme/lib/skill_runner/wrapper.py`
...
### `adminme/lib/skill_runner/pack_loader.py`
...
Add `skill.call.recorded` v2 (per prompt 04 plan — stub exists; this prompt fills in fields) and `skill.call.failed` v1 and `skill.call.suppressed` v1 to `adminme/events/schemas/`.
...
git add adminme/lib/skill_runner/ packs/skills/classify_test/ adminme/events/schemas/skill_events.py tests/
