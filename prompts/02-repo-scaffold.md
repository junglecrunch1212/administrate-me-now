...
poetry.lock                      # generated by `poetry install` — do NOT hand-create; will be produced and committed after poetry install runs
...
- **core:** `pydantic>=2.6`, `sqlalchemy>=2.0`, `alembic`, `sqlcipher3-binary`, `sqlite-vec`, `openpyxl`, `pandas`, `httpx`, `fastapi>=0.110`, `uvicorn`, `typer>=0.12`, `rich`, `textual`, `apscheduler>=3.10`, `watchdog`, `authlib`, `cryptography`, `python-dotenv`, `structlog`, `keyring`, `anthropic`, `plaid-python`, `google-api-python-client`, `google-auth`, `msgraph-sdk`, `caldav`, `vobject`.
