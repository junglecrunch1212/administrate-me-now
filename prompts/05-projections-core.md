...
Each projection lives in `adminme/projections/<name>/` and follows the pattern in REFERENCE_EXAMPLES.md §4.
...
Plus a **projection runner** (`adminme/projections/runner.py`) that:
...
This prompt emits new event types: `party.merged` v1 (stub — the pipeline that emits it is later, but the schema can be defined here so projections have something to react to when prompt 10b arrives). Add to `adminme/events/schemas/crm.py`.
...
git add adminme/projections/ tests/
