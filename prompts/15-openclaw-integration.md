...
`adminme/lib/persona_compiler.py` — reads a persona pack from `~/.adminme/packs/personas/<id>/` (at runtime — on the Mac Mini that'll be the real path; in tests, point at a fixture directory). Produces SOUL.md per REFERENCE_EXAMPLES.md §7 structure:
...
from adminme.lib.persona_compiler import compile_persona
...
git add adminme/lib/persona_compiler.py adminme/lib/openclaw_audit.py \
