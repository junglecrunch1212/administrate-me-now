...
| 11 | `11-adapter-framework.md` | **Framework expansion** per memo §3 + [D19] + [D20] + [PM-33]–[PM-35]: five abstract base classes (`CommunicationAdapter`, `ExternalStateMirrorAdapter`, `InboundDataAdapter`, `PersonalKnowledgeAdapter`, `OutboundActionAdapter` — one per epistemic-role category) + manifest format extensions (capabilities-as-list, write-capabilities, sensitivity defaults, `observation_mode_required`, owner-scope-overridability, sharing-model discriminator) + install-time validator (capability-runtime coherence, event-schema registration, projection-subscription compatibility, base-class conformance, write_capabilities method-presence, signature check) + `adminme adapters` CLI subcommand group (list, validate, scaffold under three-layer developer-mode gate) + authoring guide at `docs/adapter-authoring-guide.md`. Tier: **Introduction** (the framework prompt — most important pattern-introduction in the L1 sequence). **Pre-split candidate** — five base classes + manifest spec + validator + CLI + authoring guide is too much for one session per `D-prompt-tier-and-pattern-index.md`; split memo at orientation. | 5-6 hrs (split) | Adapter framework in place; reference-implementation prompts (11a/11b/11c-i/11e/11g) flow from it |
| 11a | `11a-gmail-plaid-central.md` | First reference implementations of `CommunicationAdapter` (Gmail) and `InboundDataAdapter` (Plaid). Both central runtime. Establishes the per-base-class adapter shape for Cat-A and Cat-C; subsequent adapters in the cohort follow this template. Taxonomy framing per [D19] / [PM-33]: each adapter declares `kind` (Cat-A / Cat-C) + `runtime` (central) + `capabilities` (list) + `sensitivity_default` + `observation_mode_required` in its pack manifest. | 3-4 hrs | Gmail + Plaid emit into event log via the new framework |
| 11b | `11b-cat-b-adapters.md` | Cat-B (External-State-Mirror) reference implementations: Apple Reminders **dual-deployment** (central variant on assistant Apple ID + bridge variant per member Apple ID) per [D18] + memo §5.4; Apple Calendar **dual-deployment** (NEW per [D22] + memo §6); Google Calendar (central, OAuth 2 + watch channels); Google Tasks (NEW central per [D18] + memo §5.5, REST API + OAuth 2). CalDAV removed per [D25] — Apple Calendar covers iCloud and Google Calendar covers Google in v1. All Cat-B adapters emit `list.*` / `list_item.*` (Reminders, Tasks, Notes-checklists) or `calendar_event.*` (Calendars) per memo §5.3 / §6.3; deduplication on `(external_id_kind, external_<entity>_id)`; sharing-model discriminator on every row. Pack manifest declares `runtime: dual` for the dual-deployment adapters with two deployment configurations. **Pre-split candidate** per memo §6.6 watch (Apple-side dual-deployment + Google-side central likely too much for one session). | 5-7 hrs (split) | Cat-B round-trip working: external state mirrored into projections + write-back where bidirectional |
| 11c-i | `11c-i-bridge-daemon-and-ingest.md` | L1-bridge infrastructure: `adminme-bridge` daemon (under `bridge/adminme_bridge/` per PM-29) + central `:3337 bridge` ingest endpoint (extends Python product APIs as fifth product per `BUILD.md` §MEMBER BRIDGES + [D17]) + connector-pack interface for non-Apple knowledge sources + `bridge.enrolled` event schema at v1. Bridge daemon and central system share event-schema models per UT-15 decision at orientation (editable install vs vendored copy). | 3-4 hrs | Bridge daemon installed on a member Mac Mini; ingest roundtrip tested with synthetic event |
| 11c-ii | `11c-ii-knowledge-adapters-and-checklist-half.md` | L1-bridge knowledge-source adapters + Apple Notes-checklists B-half. **Cat-D (Personal-Knowledge) reference implementations** of `PersonalKnowledgeAdapter`: 3 default adapters (Apple Notes prose half — `note.added` / `note.updated` / `note.deleted` events at v1 per memo §5.6 / Voice Notes — `voice_note.added` event at v1 / Obsidian — opt-in vault path) feeding `member_knowledge` projection. **Cat-B (External-State-Mirror) extension** of the Apple Notes adapter (multi-capability per [PM-35] / [D19]) — checklist detection emitting `list.*` / `list_item.*` events per [D18] + memo §5.6; AppleScript write-back for `toggle_completion` and `add_item` **only** (no remove, no in-place text edit, no reorder per [D18]). 3 system observability events for write-back outcomes per memo §5.6. Apple Notes read mechanism per UT-18 decision at orientation (recommendation: hybrid — SQLite direct for bulk + AppleScript fallback). UT-19 (last-writer-wins conflict handling) acknowledged at orientation. **Pre-split candidate (re-evaluate)** — knowledge-source adapter + checklist B-half + AppleScript write-back is plausibly two sub-prompts per memo §5.10 watch. | 4-5 hrs (split) | Personal knowledge events flowing from member bridges into event log + Notes-checklist round-trip working |
| 11d | `11d-member-knowledge-projection.md` | The 12th projection: `member_knowledge`. Subscribes to `note.*` and `voice_note.*` events emitted by 11c-ii. Tables: `notes`, `note_versions`, `voice_notes`. Owner-scope partitioned per [§1.6]; privileged content excluded per [§6.9]. Read by `vector_search` and by reactive pipelines (`commitment_extraction`, `recurrence_extraction`, `relationship_summarization`) which add `note.*` / `voice_note.*` to their subscription lists. Per memo §3.3 + UT-17 (final shape decision at projection-prompt orientation; recommendation is a new dedicated projection, already encoded in arch-summary §4 row 3.12). | 3-4 hrs | Knowledge events deterministically projected to per-member queryable state |
| 11e | `11e-contacts-adapters.md` | Cat-B contacts adapters per [D23] + memo §7. Apple Contacts (bridge per-member, Contacts.framework) + Google Contacts (central, People API). Both feed `parties.identifiers`. Closes the CRM-spine-empty-on-day-1 gap. | 3-4 hrs | Contacts feeding parties projection; CRM view populated on first bootstrap |
| 11f | `11f-lists-projection.md` | The 13th projection: `lists` per [D18] + memo §5.9. Tables: `lists`, `list_items`, `list_shares`. Subscribes to `list.*` (11 domain events) + 3 system observability events. Cross-link to `tasks` projection via `list_item.promoted_to_task` event with `source_list_item_id` provenance (the list item is unchanged by promotion). Standard cursor / rebuild / queries; the `tasks` projection's `reminder.*` subscription is retired per UT-22 closure. | 3-4 hrs | Lists materialized; round-trip from Cat-B adapters (Apple Reminders, Google Tasks, Apple Notes-checklists) working through the projection |
| 11g | `11g-home-assistant-adapter.md` | Multi-capability Cat-C + Cat-E reference implementation per [D24] + memo §8. **Cat-C state-read seam:** subscribes to HA WebSocket / `/api/states`; emits `ha.state_changed@v1`; `observation_mode_required: false` (reading is not a side-effect). **Cat-E service-call seam:** consumes `ha.service_call_requested@v1` events from the event log; calls HA's REST `POST /api/services/<domain>/<service>`; emits `action.executed@v1` (success) or `action.failed@v1` (error); `observation_mode_required: true`. When `observation_mode = active`, emits `observation.suppressed@v1` with the full would-have-sent payload and does NOT call HA's REST endpoint. Both seams ship in Phase A. The `OutboundActionAdapter` base class wraps the Cat-E seam through `adminme/lib/observation.py` per [§6.20]. | 3-4 hrs | HA state observable in event log; service-call round-trip working with observation-mode suppression |
...
| 13b | `13b-product-apis-capture-auto.md` | Capture (:3335) + Automation (:3336) FastAPI services. **Capture is a read surface, not an input pipeline (per D17)**: routers serve knowledge views (per-member + household), CRM views (Parties/Places/Assets/Accounts), recipes, semantic + structured search, and **lists views** (per [D18] + memo §5 — `/api/capture/lists` parallel to `/api/capture/parties`). Quick-capture prefix routing, voice-note ingest endpoints, and the triage queue have been retired. Automation routers gain real backing per PM-32 ADDENDUM + [D24]: `/api/automation/ha/state` (GET, returns latest cached state per entity from the Cat-C seam) and `/api/automation/ha/services` (POST emits `ha.service_call_requested` event with the body the router exposes; same shape as HA's REST `services` endpoint to keep pass-through simple). Per BUILD.md §L5-continued and architecture-summary.md §9. | 3-4 hrs | Capture read surfaces (knowledge + CRM + lists) + automation sensors with real HA backing |
...
| 14b | `14b-console-views-primary.md` | Today, Inbox, CRM, Capture, Finance, **Lists** views. **Capture view is a read surface over the personal knowledge layer + CRM (per D17)**: per-member + household knowledge views, recipes, Parties/Places/Assets/Accounts, semantic + structured search. No triage queue, no quick-capture entry, no voice-note ingest control. **CRM view is populated by Apple Contacts + Google Contacts feeds** per [D23] (closes the CRM-spine-empty-on-day-1 gap; Apple Contacts via per-member bridges, Google Contacts central). **Lists view** (NEW per [D18] + memo §5) renders the 13th projection: household-shared and per-member lists with their items, sharing model, and external-source provenance; promote-to-task affordance emits `list_item.promoted_to_task`. | 5-7 hrs | Primary surfaces (now including lists) |
...
| 16 | `16-bootstrap-wizard.md` | Ten-section Textual TUI bootstrap wizard with resumability. **§5 Assistant credentials** (per PM-32 ADDENDUM): Home Assistant token now non-optional in installs that enable HA; the optional flag remains for installs that don't run HA. **§8 Channel pairing** (Amendment-2 expansions per memo §9.3): adds four sub-steps — (a) **Lists auto-seed** per [D18] + memo §5.7 (four CoS-owned shared lists created on the assistant's Apple ID + iCloud Shared List invitations to all adult+capable-teen family-member Apple IDs; emits `list.created` ×4 + `list.share_invited` per share); (b) **Apple Calendar pairing** per [D22] + memo §6 (central variant on assistant's Apple ID; observation set up; first calendar pull tested); (c) **Google Contacts pairing** per [D23] + memo §7 (OAuth flow with assistant's Workspace; first contacts pull tested; Apple Contacts pairing prep is bridge-only and lives in §10); (d) **Home Assistant pairing** per [D24] + memo §8 (long-lived access token, REST + WebSocket connection tested, `ha.state_changed` events flowing into the event log). **§10 Bridge enrollment** per BUILD.md §MEMBER BRIDGES + arch-summary §10: central wizard generates per-member enrollment package (identity + tailnet auth key + adapter set per member with kid-bridge variant excluding Obsidian, Apple Contacts of adult contact lists, and Apple Calendar adult shared calendars); per-bridge bootstrap mini-wizard verifies macOS / iCloud signin / Tailscale / Apple Notes Full Disk Access, installs daemon under launchd, submits `bridge.enrollment_completed` to `:3337 bridge`, hands control back. **Amendment-2 §10 expansions:** the per-bridge enrollment package now includes Apple Calendar (bridge variant per [D22]) and Apple Contacts (bridge per-member per [D23]). Emits `bridge.enrolled {member_id, bridge_node_id, adapters_active}` per bridge. **Pre-split candidate (mandatory)** — the §8 + §10 Amendment-2 expansions reinforce the existing 16a / 16b-i / 16b-ii / 16c forecast; bridge enrollment likely lands as 16d or folds into 16c per `D-prompt-tier-and-pattern-index.md`. | 8-10 hrs | Bootstrap produces healthy instance + enrolled member bridges with full Amendment-2 adapter cohort paired |
| 17 | `17-cli-deploy-migrations.md` | `adminme` CLI (17 subcommand groups — adds `adminme adapters` subcommand group per [D19] / [D20] / memo §3.3: `list` (lists installed adapters by category + runtime + capabilities), `validate <pack>` (runs the install-time validator from prompt 11), `scaffold <kind>` (developer-mode-gated user-authored adapter scaffolding per [D20])), deploy scripts, migration framework. The new subcommand group exercises the adapter framework's discovery + validation surface introduced in prompt 11. | 4-5 hrs | Operator surface (now including adapter-framework affordances) |
...
**Total estimate:** ~105-141 hours of Claude Code work (Phase A), spread over 3-5 weeks of Saturday deploys (one or two prompts per slot). Breakdown: ~100-136 hrs of build prompts, ~2 hrs for the invariants document (01b), ~3-4 hrs across the four checkpoints (07.5, 10d, 14e, 15.5). Prompt 00.5 adds 1-2 hours plus any manual clipping. Prompt 19 is quick (~1 hour to write) plus whatever bootstrap time Phase B takes on the Mac Mini (typically 2-4 hours if all credentials are at hand). The Amendment-2 expansion (memo `docs/05-architecture-amendment-personal-data-layer.md` §9.3) added ~12-18 hrs over the prior 93-123 hrs estimate: net new prompts (11e Contacts, 11f lists projection, 11g Home Assistant) add ~9-12 hrs; modified prompts (11 framework expansion, 11b Cat-B cohort with Apple Calendar dual-deployment + Google Tasks central, 11c-i / 11c-ii with checklist B-half, 13b capture lists + HA routers, 14b lists view + Apple Contacts feed, 16 §8 + §10 expansions, 17 `adminme adapters` subcommand group) add ~3-6 hrs.
...
- 11a / 11b / 11c-i / 11e / 11g after 11 — reference-implementation prompts can run in parallel sessions once the framework lands. 11c-ii after 11c-i; 11d after 11c-ii; 11f after 11b for incremental visibility (11f strictly only requires the framework, but the lists projection has nothing to materialize until Cat-B adapters start emitting `list.*`).
...
- 11 before 11a / 11b / 11c-i / 11e / 11g. The framework prompt establishes five abstract base classes + manifest spec + install-time validator + `adminme adapters` CLI + authoring guide; every reference-implementation prompt in the cohort flows from it. 11c-i before 11c-ii (bridge daemon + ingest endpoint must exist before the knowledge-source adapters that emit through it); 11c-ii before 11d (the `member_knowledge` projection has nothing to consume until 11c-ii emits `note.*` and `voice_note.*` events). 11f either before or after 11b (interchangeable; recommended after 11b for incremental visibility — Cat-B adapters need a projection to write to, and the lists projection has nothing to materialize until Cat-B adapters start emitting `list.*`). 11g depends on 11 only; can run in parallel with the other reference-implementation prompts.
