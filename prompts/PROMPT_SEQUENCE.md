...
| 11c | `11c-bridge-and-knowledge-adapters.md` | L1-bridge: `adminme-bridge` daemon + `:3337 bridge` ingest endpoint + 3 default knowledge-source adapters (Apple Notes, Voice Notes, Obsidian) + 5 new event schemas at v1 (`note.added`, `note.updated`, `note.deleted`, `voice_note.added`, `bridge.enrolled`) + connector-pack interface for non-Apple knowledge sources. Bridge daemon code lives under `bridge/` per PM-29; central ingest endpoint extends Python product APIs as fifth product `:3337` per BUILD.md §MEMBER BRIDGES + D17. **Pre-split candidate** — split into 11c-i (bridge daemon + ingest endpoint + connector-pack interface) and 11c-ii (3 default adapters + event schemas + tests) per memo §5.2. | 5-6 hrs (split) | Personal knowledge events flowing from member bridges into event log |
| 11d | `11d-member-knowledge-projection.md` | The 12th projection: `member_knowledge`. Subscribes to `note.*` and `voice_note.*` events emitted by 11c. Tables: `notes`, `note_versions`, `voice_notes`. Owner-scope partitioned per [§1.6]; privileged content excluded per [§6.9]. Read by `vector_search` and by reactive pipelines (`commitment_extraction`, `recurrence_extraction`, `relationship_summarization`) which add `note.*` / `voice_note.*` to their subscription lists. Per memo §3.3 + UT-17 (final shape decision at projection-prompt orientation; recommendation is a new dedicated projection, already encoded in arch-summary §4 row 3.12). | 3-4 hrs | Knowledge events deterministically projected to per-member queryable state |
...
| 13b | `13b-product-apis-capture-auto.md` | Capture (:3335) + Automation (:3336) FastAPI services. **Capture is a read surface, not an input pipeline (per D17)**: routers serve knowledge views (per-member + household), CRM views (Parties/Places/Assets/Accounts), recipes, and semantic + structured search. Quick-capture prefix routing, voice-note ingest endpoints, and the triage queue have been retired. Per BUILD.md §L5-continued and architecture-summary.md §9. | 3-4 hrs | Capture read surfaces + automation sensors |
...
| 14b | `14b-console-views-primary.md` | Today, Inbox, CRM, Capture, Finance views. **Capture view is a read surface over the personal knowledge layer + CRM (per D17)**: per-member + household knowledge views, recipes, Parties/Places/Assets/Accounts, semantic + structured search. No triage queue, no quick-capture entry, no voice-note ingest control. | 5-6 hrs | Primary surfaces |
...
| 16 | `16-bootstrap-wizard.md` | Ten-section Textual TUI bootstrap wizard with resumability. §10 Bridge enrollment per BUILD.md §MEMBER BRIDGES + arch-summary §10: central wizard generates per-member enrollment package (identity + tailnet auth key + adapter set per member with kid-bridge variant excluding Obsidian + bridge bootstrap mini-wizard); operator copies package to each bridge Mac Mini; mini-wizard verifies macOS / iCloud signin / Tailscale / Apple Notes Full Disk Access, installs daemon under launchd, submits `bridge.enrollment_completed` to `:3337 bridge`, hands control back. Emits `bridge.enrolled {member_id, bridge_node_id, adapters_active}` per bridge. **Pre-split candidate (mandatory)** — the bridge-enrollment addition reinforces the existing 16a / 16b-i / 16b-ii / 16c forecast; bridge enrollment likely lands as 16d or folds into 16c per `D-prompt-tier-and-pattern-index.md`. | 7-9 hrs | Bootstrap produces healthy instance + enrolled member bridges |
...
- 11 before 11c; 11c before 11d. Standalone Python adapters establish the adapter pattern before bridge-side adapters extend it; 11d's `member_knowledge` projection has nothing to consume until 11c emits `note.*` and `voice_note.*` events. The future 11→11a/11b split (per `D-prompt-tier-and-pattern-index.md`) does not change this: 11c and 11d follow whichever adapter prompts have shipped, then proceed in order.
