...
3. If they differ → it's a real OpenClaw API change. Update the wrapper (in `adminme/lib/skill_runner/wrapper.py::_parse_openclaw_response`) to handle the new shape. Keep backward compatibility with a version detection on startup.
