...
You are Claude Code Opus 4.7 running in a terminal on a fresh build machine. Build **AdministrateMe** (code identifier: `adminme`) — a self-hostable, long-lived, event-sourced household operating system. It is simultaneously:
...
Work phase by phase. At the end of each phase, run all tests, produce a summary of what you built and what the tests said, and stop for human review before proceeding.
...
- "What OpenClaw docs do I need to read first?" → BUILD.md "BEFORE YOU START: LEARN OPENCLAW" section
...
When the files appear to disagree: **BUILD.md wins for contracts, CONSOLE_PATTERNS.md wins for Node implementation details, CONSOLE_REFERENCE.html wins for visual + interaction design, REFERENCE_EXAMPLES.md wins for pack shape, DIAGRAMS.md wins for nothing — it's mental-model formation, always defer to the specs.** If a genuine contradiction appears, flag and ask before writing code.

---

## CRITICAL OPERATING RULES — READ FIRST, REFERENCE CONSTANTLY

1. **Read before you write.** Before writing code that touches OpenClaw, Plaid, Tailscale, BlueBubbles, Apple EventKit, or Anthropic's API, fetch the relevant documentation via `WebFetch`. Do not rely on training data for specifics — these APIs change.

2. **Verify, do not assume.** When you think you know a CLI flag, config key, tool signature, schema, or event format, check the docs. The cost of a WebFetch is nothing compared to the cost of a silent wrong assumption propagating through an event-sourced codebase.

3. **Tenant-agnostic codebase.** No family name, person's name, address, phone number, email, account number, or medical detail appears anywhere in platform code. All such data lives in the instance directory (`~/.adminme/`), populated by the bootstrap wizard. A static identity scan test runs from Phase 0 and fails CI if any hardcoded tenant data slips into platform code or tests (except explicitly-marked fixtures under `tests/fixtures/`).

4. **The event log is the source of truth.** Projections — including xlsx workbooks, SQLite read tables, dashboard caches — are derived. Never write to a projection except through a projector. Never mutate an event after it is recorded.

5. **Propose, then commit, for LLM-authored state changes.** Pipelines running LLM skills emit *proposal events* (`CommitmentProposed`, `TaskProposed`, `PartyMergeSuggested`). Humans approve via the inbox; approval emits a confirmation event (`CommitmentConfirmed`); projections update from the confirmed event. Direct LLM writes to "live" state are prohibited. Deterministic adapters and deterministic pipelines may write directly (e.g., `MessageReceived` is direct from the IMAP adapter; no human approval needed to record that a message arrived).

6. **Observation mode is first-class and default-on.** At bootstrap, `ADMINME_OBSERVATION_MODE=true`. Every outbound adapter (email send, iMessage send, SMS send, voice call, physical mail, Privacy.com charge, Apple Reminders write, Plaid-triggered alert) and every proactive pipeline (digest, reward, paralysis, reminder dispatch) checks observation mode before firing externally. In observation mode the action is composed and recorded as an event (`OutboundSuppressed`), not fired. The tenant flips observation off after 5-7 days of review.

7. **Identity-first privacy.** Privileged work content is never shared to the assistant's accounts (Apple ID, Google Workspace), so it never enters the ingest layer. This is the primary privacy boundary. Session-scope (`dmScope: per-channel-peer` in OpenClaw) is secondary. Event-level `sensitivity: privileged` with `visibility_scope` narrowing is tertiary. All three layers must be present.

8. **Ask before destructive operations.** Never delete, drop, truncate, reset, or uninstall without a dry-run preview first, even during development.

9. **Validate at every boundary.** All events, API inputs, API outputs, skill inputs, and skill outputs use Pydantic v2 models. Open enums use a sentinel pattern so plugins can extend. No `dict[str, Any]` crossing a layer boundary.

10. **Children are data-model members, not users.** Kids exist in the Party table (with `kind: person`, age, Memberships to the household). They do not receive agent bindings. They do not receive outbound messages. They interact only through display-only surfaces (scoreboard) and parent-mediated captures.

11. **Coparents are Parties of kind `person` with `Relationship(label: parent_of, party_b: child, direction: →)`.** Their phones are in channel allowlists so their messages don't bounce. Their messages create events and projections, but they receive no autonomous outbound; all replies go through an inbox approval.

12. **Mode 1 only for sending.** The assistant sends as itself (persona handle). Mode 2 (draft-for-principal-to-send) is routed to the principal's approval inbox. Mode 3 (auto-send as a principal) is prohibited.

13. **Never reference private medical, legal, or financial details to outside parties.** Enforced at the outbound adapter layer via a final content-filter pass that checks for PII leakage using a dedicated skill.

14. **Never describe social institutions, schools, or clubs as prestigious or exclusive.** Enforced at the voice layer (persona `voice.md`) and at the final outbound filter.

15. **Profile packs and persona packs are the unit of customization.** Profile = bundle assigned to a member (views + engines + tuning). Persona = bundle defining the agent's identity (one per instance). Both are installable, versionable, shareable via a registry. Neither can modify the event log schema or the authority model.

16. **Platform repo contains no tenant data.** Running the bootstrap wizard twice with different `ADMINME_INSTANCE_DIR` values produces two fully independent instances on the same machine. CI verifies this via isolation canary tests.

17. **Migration framework from day one.** Every schema change ships a numbered migration. Migrations are idempotent. Existing tenants can always upgrade without data loss. Event schemas get `schema_version` + upcaster functions. This is non-negotiable for a platform that will exist for 10+ years.

18. **Stop between phases.** After each phase commit, report what was built, what tests passed, what was deferred. Wait for explicit human approval before proceeding.

19. **Prefer deterministic over LLM.** When there is a rule to encode, encode it as code + config, not as a prompt. LLM is for fuzzy composition (draft emails, digest narrative, classification, extraction). Everything else is deterministic. This is especially important for paralysis templates, reward dispatch, and any adversarial-safe surface.

20. **Prefer extensibility over features.** If you catch yourself writing a switch statement on entity type, or an `if member_name == "James"` check, or an embedded SQL query in pipeline code — stop. That's a sign something wants to be an open enum, a plugin table, a Party attribute, or a projection.
...
**Do this before writing any code.** Execute these `WebFetch` calls in order. Take notes in `docs/openclaw-notes.md` (max 600 lines — if you go longer, you're copying rather than summarizing). This file is your reference for the rest of the build. When later phases ask you to "register a slash command" or "install a skill" or "configure a standing order," you will need these docs in your working memory.

**Required OpenClaw reading (in order):**

1. `https://docs.openclaw.ai/` — Overview
2. `https://docs.openclaw.ai/start/openclaw` — Personal assistant setup. Read carefully: AdministrateMe presents itself to OpenClaw as a Personal Assistant on a Mac Mini, upgraded into a Chief of Staff via skills, tools, plugins, slash commands. This is the mental model for the entire build.
3. `https://docs.openclaw.ai/install` — Installation. Bootstrap wizard §1 checks that OpenClaw is installed before proceeding.
4. `https://docs.openclaw.ai/concepts/architecture` — Gateway architecture
5. `https://docs.openclaw.ai/concepts/agent-workspace` — Workspace layout. AdministrateMe's workspace lives at `~/Chief` by convention.
6. `https://docs.openclaw.ai/concepts/soul` — SOUL.md personality guide. AdministrateMe's persona pack (REFERENCE_EXAMPLES.md §7) maps to this.
7. `https://docs.openclaw.ai/concepts/agent-loop` — How an agent turn works
8. `https://docs.openclaw.ai/concepts/memory` — Memory system. Understand the boundary with AdministrateMe's event log (see above).
9. `https://docs.openclaw.ai/concepts/session` — Session management. **Critical:** `dmScope: per-channel-peer` for multi-principal households.
10. `https://docs.openclaw.ai/concepts/multi-agent` — Multi-agent routing. AdministrateMe's assistant roster (persona-primary + subagents for specific domains) is multi-agent.
11. `https://docs.openclaw.ai/tools` — Built-in tools overview
12. `https://docs.openclaw.ai/tools/skills` — **Skills system.** This is the mechanism that upgrades a Personal Assistant into a Chief of Staff. AdministrateMe's skill packs are OpenClaw skills.
13. `https://docs.openclaw.ai/tools/creating-skills` — How to build skills (SKILL.md + handler + tests). Compare to REFERENCE_EXAMPLES.md §3 which shows an AdministrateMe skill pack filled in.
14. `https://docs.openclaw.ai/tools/slash-commands` — Slash command reference. AdministrateMe registers: `/digest`, `/whatnow`, `/capture`, `/comms`, `/bill`, `/approve`, `/crm`, `/commit`, `/review`, `/scoreboard`.
15. `https://docs.openclaw.ai/tools/plugin` — Plugin system. AdministrateMe installs its own plugins: `openclaw-memory-bridge` (ingests OpenClaw memory as events), `adminme-projection-reader` (lets OpenClaw skills read projections), `adminme-event-emitter` (lets OpenClaw skills emit events).
16. `https://docs.openclaw.ai/tools/exec` — Exec tool
17. `https://docs.openclaw.ai/tools/exec-approvals` — Approval gates. These map to AdministrateMe's governance action_gates; the two systems must agree.
18. `https://docs.openclaw.ai/tools/elevated` — Elevated mode
19. `https://docs.openclaw.ai/gateway/security` — Security model
20. `https://docs.openclaw.ai/gateway/sandboxing` — Sandboxing
21. `https://docs.openclaw.ai/gateway/configuration` — Configuration reference
22. `https://docs.openclaw.ai/gateway/heartbeat` — Heartbeat
23. `https://docs.openclaw.ai/automation/cron-jobs` — Cron
24. `https://docs.openclaw.ai/automation/hooks` — Hooks
25. `https://docs.openclaw.ai/automation/standing-orders` — Standing orders. This is how AdministrateMe's proactive behaviors fire (paralysis detection, morning digest, reward dispatch, etc.).
26. `https://docs.openclaw.ai/tools/subagents` — Sub-agents
27. `https://docs.openclaw.ai/channels/bluebubbles` — BlueBubbles (iMessage on Mac). The assistant signs into its own Apple ID on the Mac Mini.
28. `https://docs.openclaw.ai/channels/telegram` — Telegram
29. `https://docs.openclaw.ai/channels/discord` — Discord
30. `https://docs.openclaw.ai/channels/pairing` — Channel pairing
31. `https://docs.openclaw.ai/gateway/protocol` — WS protocol
32. `https://docs.openclaw.ai/nodes` — Nodes. Family members' laptops and phones pair as nodes.
33. `https://docs.openclaw.ai/platforms/macos` — macOS companion app
34. `https://docs.openclaw.ai/tools/clawhub` — ClawHub skill registry. AdministrateMe installs from ClawHub: `apple-reminders`, `apple-notes`, `apple-contacts`, `apple-mail-search`, `findmy-location`, `home-assistant`, `tavily-search`, `summarize`, `mac-tts`, `elevenlabs-skill`, `audio-transcribe`, `healthkit-sync`.
35. `https://docs.openclaw.ai/cli` — CLI reference

**Optional but recommended:**

- `https://docs.openclaw.ai/concepts/dreaming` — Memory consolidation
- `https://docs.openclaw.ai/concepts/compaction` — Context compaction
- `https://docs.openclaw.ai/automation/webhook` — Webhooks
- `https://github.com/openclaw/openclaw` — Source repo README

**Additional platform-specific reading (do after OpenClaw):**

- `https://plaid.com/docs/` — Plaid API (Link, Transactions, Balance, Identity, Investments, Liabilities)
- `https://tailscale.com/kb/1223/tailscale-serve` — Tailscale Serve
- `https://tailscale.com/kb/1223/funnel` — Tailscale Funnel (for Plaid webhooks)
- `https://bluebubbles.app/docs/` — BlueBubbles server documentation
- `https://developer.apple.com/documentation/eventkit` — EventKit (Apple Reminders + Calendar)
- `https://docs.claude.com/` — Anthropic API (Claude Code, SDK, streaming, tool use)

Write `docs/openclaw-notes.md` (max 600 lines), `docs/plaid-notes.md` (max 300), `docs/apple-integrations.md` (max 300), `docs/tailscale-notes.md` (max 200). These are your reference for the rest of the build. If you skip this step and try to work from training data, you will be wrong about specifics and your code will fail at integration time.

**One-page OpenClaw cheat sheet to produce as output of this phase.** After reading the 35 URLs, produce `docs/openclaw-cheatsheet.md` (max 100 lines) that answers:
...
5. What is the exact shape of a SKILL.md file that OpenClaw accepts?
6. What is `dmScope: per-channel-peer` and when does it apply vs. `shared`?
7. How does the gateway's approval-gates system interact with guardedWrite (CONSOLE_PATTERNS.md §3)?
8. Where does OpenClaw store its state, and does any of it need to be backed up (vs. derived from `~/.adminme/`)?

This cheat sheet is referenced in later phases. Do not proceed to PHASE 0 without it.
...
Pin exact versions in `pyproject.toml`. Use `uv` for dependency management.
...
- A Pydantic payload model registered in `platform/lib/event_types/<namespace>/<type>.py`
...
A Python daemon (`platform/daemons/xlsx_sync/forward.py`) subscribes to the event bus, filters to event types that affect either workbook, and debounces regenerations.
...
A separate daemon (`platform/daemons/xlsx_sync/reverse.py`) watches the two files via `watchdog` (inotify on Linux, FSEvents on macOS).
...
Fixture tests in `platform/projections/xlsx_workbooks/tests/`:
...
Each pipeline lives in `platform/pipelines/<namespace>/<name>/`:
...
5. Invoke OpenClaw's skill execution endpoint: `POST http://127.0.0.1:18789/skills/invoke` with `{skill_name, inputs, correlation_id, session_context, dmScope}`. OpenClaw handles prompt rendering, provider selection, LLM call with retries, and token accounting.
...
Standalone Python adapters are loaded via `adminme.adapters` entry point → built-in adapters live in `platform/adapters/<family>/<n>/`. OpenClaw-plugin adapters are registered via OpenClaw's plugin system and live in `platform/openclaw-plugins/<n>/`.
...
The Node console lives at `platform/console/`. Port 3330. Express server serving `shell.html` and proxying to Python product APIs.
...
The Node/Express code for the platform-critical console patterns is specified in the companion file **CONSOLE_PATTERNS.md**. Read that file before writing any code in `platform/console/`. The patterns it covers are:
...
│       └── …
├── logs/
│   ├── platform.jsonl                        # correlation-ID-stamped
│   ├── adapters.jsonl
│   ├── pipelines.jsonl
│   ├── skills.jsonl
│   └── console.jsonl
...
- Logs in `~/.adminme/logs/platform.jsonl` show correlation IDs flowing end-to-end — one inbound message → one classify skill call → one extract skill call → one commitment.proposed event → one inbox surface update.
...
3. **Identity scan test** (`tests/unit/test_no_hardcoded_identity.py`). Fails CI if platform/bootstrap/profiles/personas/integrations/tests contain literal email patterns, E.164 phones, street-address patterns, or tenant-name blocklist entries.

4. **Layer violation scan test** (`tests/unit/test_layer_violations.py`). Fails if any `platform/pipelines/<X>/` imports from `platform/pipelines/<Y>/`, or any `platform/products/<X>/` imports from `platform/products/<Y>/`, or any code outside `platform/lib/event_log/` writes to `events.db`.

5. **Append-only event log test**. Fails if any code path outside `platform/lib/event_log/` has `UPDATE events` or `DELETE FROM events` or equivalent.
...
7. uv setup with pinned deps. Node install at `platform/console/`.

8. CI pipeline: lint → type-check (pyright strict) → unit → integration → canary → coverage gate (85% on core).

9. Verify: `uv run pytest --collect-only`, `cd platform/console && npm install`, identity scan passes, layer scan passes, append-only scan passes.
...
3. **`platform/lib/observation_mode/`:** 
   - `is_active() -> bool`
...
- [ ] All required WebFetches completed; docs/openclaw-notes.md, docs/plaid-notes.md, docs/apple-integrations.md written
- [ ] Identity static scan passes across all of platform/, bootstrap/, profiles/, personas/, integrations/, tests/ (fixtures excluded)
...
- **WebFetch** — read every doc before writing code that depends on it. Do not rely on training data for specifics of OpenClaw, Plaid, Tailscale, BlueBubbles, EventKit, or Anthropic's API.
...
1. Fetch the OpenClaw docs (all 35 URLs from the prior list), Plaid docs, Tailscale Serve/Funnel docs, BlueBubbles server docs, EventKit docs, Anthropic API docs.
2. Write `docs/openclaw-notes.md` (max 600 lines), `docs/plaid-notes.md` (max 300), `docs/apple-integrations.md` (max 300). These are your reference for the rest of the build.
3. Scaffold the repo structure.
4. Implement the three static scans (identity, layer, append-only).
5. Stop and report.

After Phase 0's scaffolding + scans are in place, confirm that the platform skeleton is sound before starting Phase 1. I want to see a clean test suite and a clean scan report.
