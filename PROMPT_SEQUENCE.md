...
Rationale: docs sites increasingly block bot traffic (Cloudflare, captcha walls, 403s to non-browser user agents). If the build depended on live fetches, it would fail unpredictably when a site blocked Claude Code's fetcher. The mirror eliminates that failure mode entirely. The tradeoff — docs can go stale — is managed by `docs/reference/_refresh-schedule.md` which lists a re-fetch cadence per section.

**If a prompt says "per OpenClaw docs" or "per Plaid docs":** read from `docs/reference/<section>/`. If the referenced file doesn't exist, the mirror is incomplete — stop, complete prompt 00.5 (including any manual clipping), and then resume.
...
| 00.5 | `00.5-mirror-docs.md` | Mirror external docs (OpenClaw, Plaid, BlueBubbles, Google, Apple, Tailscale, Textual, etc.) into `docs/reference/` so the build never depends on live fetches | 1-2 hrs | Mirror complete + failures checklist |
...
**Total estimate:** 88-118 hours of Claude Code work (Phase A), spread over 2-4 weeks of Saturday deploys (one or two prompts per slot). Prompt 00.5 adds 1-2 hours plus any manual clipping. Prompt 19 is quick (~1 hour) plus whatever bootstrap time Phase B takes on the Mac Mini (typically 2-4 hours if all credentials are at hand).
...
> **Phase + repository + documentation discipline.**
>
...
>
> Before reading any artifact or committing any code, run `git pull origin main` to ensure you have the operator's latest changes.
>
...
>
> When a prompt says "per OpenClaw docs" or "per Plaid docs" or any external documentation reference, read from `docs/reference/<section>/` — the local mirror populated by prompt 00.5. **Do NOT use WebFetch to pull these docs live.** The mirror is the version-pinned source of truth. If a referenced file is missing from `docs/reference/`, stop the session and report that prompt 00.5 is incomplete.
>
> Commit your work frequently with honest messages. Work on a branch named per the prompt (`phase-NN-<slug>`), push the branch, and let the operator open the PR — do not push directly to `main`.
